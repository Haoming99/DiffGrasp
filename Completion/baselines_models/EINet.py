...
class FeaExtract(nn.Module):
    def __init__(self, dim_feat=384, num_seeds=512, seed_fea=256):
        ...
        self.sa_module_1 = PointNet_SA_Module_KNN(num_seeds, 16, 3, [32, seed_fea], group_all=False, if_bn=False,
                                                  if_idx=True)
        self.transformer_1 = Transformer(seed_fea, dim=64)
        self.sa_module_2 = PointNet_SA_Module_KNN(num_seeds // 4, 16, 128, [128, 256], group_all=False, if_bn=False,
                                                  if_idx=True)
...
class EINet(nn.Module):
    def __init__(self, config):
        ...
        self.feat_extractor = FeaExtract(dim_feat=config.dim_feat, num_seeds=config.num_seeds,
                                         seed_fea=config.dim_seed_fea)
