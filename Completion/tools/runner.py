...
Test_data_dir = '/home/haoming/Downloads/data/input/*/test'
Test_pcd_dir = '/home/haoming/Downloads/data/gt'
Train_pcd_dir = '/home/haoming/Downloads/data/gt'
Train_data_dir = '/home/haoming/Downloads/data/input/*/train'
...
train_loader = torch.utils.data.DataLoader(train_data, batch_size=48, shuffle=True, num_workers=16)#bs = 256
...
def test(base_model, test_loader, ChamferDisL1, ChamferDisL2, args, config, logger=None):
    ...
    with torch.no_grad():
        ...
        print("Partial Point Cloud Before Preprocessing: ", partial_.shape)
        min_coords = np.min(partial_, axis=0)
        max_coords = np.max(partial_, axis=0)
        ...
        print("Partial Point Cloud After FPS: ", partial.shape)
        min_coords = np.min(partial, axis=0)
        max_coords = np.max(partial, axis=0)
        print("min_coords: ", min_coords)
        print("max_coords: ", max_coords)
        ...
        a = partial.reshape(2048, 3)
        print("Partial Point CLoud Normalized: ", a.shape)
        ...
        # dense_points = ret[3] for EINet
        print("Number of points: ", dense_points.shape)

        pcd = dense_points.cpu().squeeze()#[8192, 3]

        #chamferl1 = ChamferDisL1(pcd, gt)
        ...
        o3d.io.write_point_cloud('/home/haoming/Downloads/3DSGrasp-master/tmp_data/complete_pc.pcd',pcdd)
        ...
        red_color = np.array([[0, 0, 1] for _ in range(len(pcddx.points))])  # RGB: Red

        # Assign the color to the point cloud
        pcddx.colors = o3d.utility.Vector3dVector(red_color)

        o3d.io.write_point_cloud('/home/haoming/Downloads/3DSGrasp-master/tmp_data/complete_pc_x.pcd', pcddx)
