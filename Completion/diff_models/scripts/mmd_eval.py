...
try:
    from libs.structural_losses.tf_nndistance import nn_distance
    from libs.structural_losses.tf_approxmatch import approx_match, match_cost
...
Shapenet_ONet_Base = '/Datasets/ShapeNet/'
...
def collect_src_pcs(args):
    ...
    all_paths = glob(os.path.join(args.src, "*/fake-z*.ply"))
...
def collect_shapenet_ref_pcs(dataset_base, class_name, num_pts=2048):
    classes = snc_synth_category_to_id[class_name]
    if not isinstance(classes, list):
        classes = [classes]
    ref_pcs = list()
    for cls_idx in classes:
        with open(osp.join(dataset_base, cls_idx, 'cgan_test.lst')) as f:
            model_ids = f.read().split('\n')
        for model_id in tqdm(model_ids, desc=f"loading {cls_idx}"):
            model_path = osp.join(dataset_base, cls_idx, model_id)
            points_dict = dict(**np.load(osp.join(model_path, 'points.npz')))
            cur_pcs = (points_dict['points'] + points_dict['loc']) * points_dict['scale']
            
            sampled_pcs = sample_point_cloud_by_n(cur_pcs, num_pts)
            ref_pcs.append(sampled_pcs)
    ref_pcs = np.stack(ref_pcs, axis=0)
    print("reference point clouds: {}".format(ref_pcs.shape))
    return ref_pcs

def load_gen_pcs(gen_paths, num_pts=2048, load_mesh=False):
    """collect completion results: generated point clouds"""
    ...
    for path in tqdm(gen_paths):
        if load_mesh:
            ...
            sample_pts, _ = trimesh.sample.sample_surface(mesh, num_pts)
            # sample_pts = sample_point_cloud_by_n(sample_pts, num_pts) # This is wrong because we are sampling from a mesh
        else:
            # pcs = np.loadtxt(path)
            pcs = np.load(path)
            sample_pts =  sample_point_cloud_by_n(pcs, num_pts)
        gen_pcs.append(sample_pts)
...
def main():
    ...
    parser.add_argument("src", type=str, help='path to completion results')
    parser.add_argument("--dataset", type=str, choices=['3depn', 'partnet', 'partnet_scan', 'shapenet'], required=True)
    parser.add_argument('-g', '--gpu_ids', type=str, default=None, help="gpu to use, e.g. 0  0,1,2. CPU not supported.")
    parser.add_argument("--class_name", type=str)
    parser.add_argument("--eval_cls", type=str, required=True, help='Category to evaluate [chair, table, airplane] for ShapeNet etc')
    ...
    parser.add_argument("--load_mesh", action='store_true', help="Load mesh instead of point cloud saved in advance")
    args = parser.parse_args()

    # os.environ["CUDA_VISIBLE_DEVICES"] = str(args.gpu_ids)

    if args.output is None:
        args.output = args.src + f'mmd-{args.class_name}-{args.eval_cls}_{args.dist_id}.txt'

    if args.dataset == '3depn':
        ref_pcs = collect_shapenet_test_set_pcs(args)
    if args.dataset == 'shapenet':
        ref_pcs = collect_shapenet_ref_pcs(Shapenet_ONet_Base, args.class_name)
    ...
    gen_class_id = snc_synth_category_to_id[args.eval_cls]
    if args.load_mesh:
        gen_pcs_paths = sorted(glob(os.path.join(args.src, 'meshes', f"{gen_class_id}/*_[0-9]*.obj")))
    else:
        # gen_pcs_paths = sorted(glob(os.path.join(args.src, 'points', f"{gen_class_id}/*_[0-9]*.pts.txt"))) # Will be depracted soon
        gen_pcs_paths = sorted(glob(os.path.join(args.src, 'points', f"{gen_class_id}/*_[0-9]*.pts.npy")))

    chunk_size = int(math.ceil(len(gen_pcs_paths) / args.dist_size))
    chunked_gen_paths = gen_pcs_paths[args.dist_id * chunk_size: (args.dist_id + 1) * chunk_size]

    # sample_pcs = collect_src_pcs(args)
    gen_pcs = load_gen_pcs(chunked_gen_paths)

    mmd, matched_dists = minimum_mathing_distance(gen_pcs, ref_pcs, args.batch_size)
