...
def main():
    ...
    out_dir = f'/home/wen/paper_vis/{exp_name}/preview-allcat/'
    task_list = []

    for cat_id in cgan_all:
        os.makedirs(f"{out_dir}/{cat_id}", exist_ok=True)
        split_file = f'/Datasets/ShapeNet/{cat_id}/cgan_val.lst'
    ...
    vis_camera = dict(camPos=np.array([1, 1, 1]), camLookat=np.array([0., 0., 0.]),
                    camUp=np.array([0, 1, 0]), camHeight=1, resolution=resolution, samples=sample_times, light_samples=sample_times)
    ...
    for cat_id, model_id in tqdm(chunked_tasks, desc="Main Task"):
        try:
            ...
            auto_fns = [f"{auto_base}/{cat_id}/{model_id}_{b_idx:02d}.jpg" for b_idx in range(3)]
            for auto_fn in auto_fns:
                res_img = np.zeros((resolution[0], resolution[1], 3), dtype=np.uint8)
                try:
                    auto_im = plt.imread(auto_fn)
                    res_img += auto_im
            ...
            grid_img = torchvision.utils.make_grid(all_imgs, 4)
