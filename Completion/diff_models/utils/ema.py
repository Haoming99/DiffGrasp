...
class EMA(pl.Callback):
    ...
    Implementation detail:
        ...
        - When multi gpu, we broadcast ema weights and the original weights in order to only hold 1 copy in memory.
          ...
          performance.
    """
    def __init__(self, decay: float = 0.9999, ema_device: Optional[Union[torch.device, str]] = None, pin_memory=True, val_with_ema=True):
        super().__init__()
        self.decay = decay
        ...
        self.val_with_ema = val_with_ema
    ...
    def on_train_start(self, trainer: "pl.Trainer", pl_module: pl.LightningModule) -> None:
        ...
        if not self._ema_state_dict_ready and pl_module.global_rank == 0:
            ...
            self.ema_state_dict = {k: v.clone().detach() for k, v in self.get_state_dict(pl_module).items()}
            if self.ema_device:
    ...
    def on_load_checkpoint(
        ...
        if self.ema_device and next(iter(self.ema_state_dict.values())).device != torch.device(self.ema_device):
            self.ema_state_dict = {k: tensor.to(device=self.ema_device) for k, tensor in self.ema_state_dict.items()}