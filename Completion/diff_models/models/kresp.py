...
class KResPrior(KiloBase):
    ...
    def __init__(self, decoder: nn.Module, partial_global_pointnet: nn.Module, partial_local_pointnet: nn.Module,
        ...
        self.local_vae_encoders = nn.ModuleList([GroupFCEncoder(in_channels=self.feat_dim,
                                      hidden_channels=self.global_latent_dim*2,
                                      out_channels=self.global_latent_dim*2, groups=self.Rc * cur_reso * 3,
        ...
        self.local_prior_encoders = nn.ModuleList([GroupFCEncoder(in_channels=self.global_latent_dim,
                                      hidden_channels=self.global_latent_dim*2,
                                      out_channels=self.global_latent_dim*3, groups=self.Rc * cur_reso * 3,
    ...
    def step(self, batch, batch_idx):
        ...
        for idx, (cur_reso, cur_x, cur_vae_encoder, cur_prior_encoder) in enumerate(zip(self.resolutions[1:], layered_x, self.local_vae_encoders, self.local_prior_encoders)):
            ...
            local_mus, local_log_vars = cur_vae_encoder(x=cur_x, c=repeated_latents).chunk(2, dim=1)
            local_stds = torch.exp(local_log_vars / 2)
            local_qs = torch.distributions.Normal(local_mus, local_stds)

            local_zs = local_qs.rsample()
 
            prior_feats = cur_prior_encoder(x=repeated_latents, c=None)
            local_prior_mus, local_prior_log_vars, xpp = prior_feats[:, :self.global_latent_dim], prior_feats[:, self.global_latent_dim:self.global_latent_dim*2], prior_feats[:, self.global_latent_dim*2:]
        
            repeated_latents = repeated_latents + xpp
