...
class DiffVQVAE(KiloBase):
    ...
    def __init__(self, decoder: nn.Module, local_pointnet: nn.Module, unet3d: UNet3DCLI = None,
        ...
        self.vqvae = VQModel(in_channels=feat_dim, out_channels=feat_dim, down_block_types=down_block_types,
    ...
    def step(self, batch, batch_idx):
        ...
        for plane_type in self.plane_types:
            h = self.vqvae.encode(x_local[plane_type]).latents
            quant, cur_emb_loss, info = self.vqvae.quantize(h) 
            pred_feats[plane_type] = self.vqvae.decode(quant, force_not_quantize=True).sample # have already been quantized to obtain the loss
            emb_losses[plane_type] = cur_emb_loss
        ...
        emb_loss = sum(v for v in emb_losses.values()) / len(emb_losses)
        ...
        logs.update({f"emb-{k}": v.mean() for k, v in emb_losses.items()})
    ...
    def encode_inputs(self, partial_pcs, full_pcs=None):
        ...
        if full_pcs is not None:
            x_local = self.local_pointnet(full_pcs)
            for plane_type in self.plane_types:
                pred_feats[plane_type] = self.vqvae(x_local[plane_type]).sample

        else:
            for plane_type in self.plane_types:
                rand_idxs = torch.randint(0, self.vqvae.quantize.n_e, (bs, 1, latent_plane_dim, latent_plane_dim))
                flat_idxs = rearrange(rand_idxs, 'b 1 v1 v2 -> (b v1 v2)') 
                flat_feats = self.vqvae.quantize.embedding.weight.data[flat_idxs].detach()
                rand_latents = rearrange(flat_feats, '(b v1 v2) f -> b f v1 v2', 
                    b=bs, f=self.vqvae.quantize.e_dim, v1=latent_plane_dim, v2=latent_plane_dim)
                pred_feats[plane_type] = self.vqvae.decode(rand_latents).sample
