# have_fl.py: HAVE with factorization layer
# adapted from krlocal_cat
# Add support for kl_reg_loss and attn grid 
# Would add edr tv loss etc.

from http.client import PRECONDITION_FAILED # what's the use of this?
...
class HAVE_FL(KiloBase):
    ...
    def __init__(self, decoder: nn.Module, global_pointnet: nn.Module, local_pointnet: nn.Module,
        ...
        feat_dim=16, vox_reso=16, kl_weight=0.01, local_kl_weight=0.01, recon_weight=1., tv_weight=0.0, reg_weight=0.01, edr_weight=0.0,
        Rc=10, global_latent_dim=16, local_cond_dim=16, vd_init=False, fuse_feats=False, cat_grid=False,
        dec_pre_path: Optional[str]=None, residual_latents=True, resolutions=[1, 4, 8, 16], attn_grid=False,
        kl_reg_loss_weight=0, impute_cond=False, sample_temperature=None,
        ...
        test_num_pts=2048, ed_pre_path=None, ende_lr=None, num_fc_layers=3):
        ...
        if sample_temperature is not None:
            self.register_buffer("sample_temperature", torch.tensor(sample_temperature), persistent=False)
        ...
        self.impute_cond = impute_cond # "impute" the conditional feature map to the decoder to improve fidelity
        # This makes the network depends on non-homogenous features for reconstruction
        # grid for full shape and tri-planes for partial observation
    ...
    def step(self, batch, batch_idx):
        ...
        if self.attn_grid:
            pred_feats["grid_c"] = c_local["grid"]

        if self.impute_cond:
            for k in ['xy', 'xz', 'yz']:
                pred_feats[k] = c_local[k]
        ...
        if self.reg_weight > 0:
            reg_loss = torch.linalg.norm(sampled_feat_grid)
        else:
            reg_loss = 0

        if self.edr_weight > 0:
            ...
            edr_initial_pred_occ = self.decode(initial_coords, z=pred_feats, c=c_global)
            edr_offset_pred_occ = self.decode(offset_coords, z=pred_feats, c=c_global)
        ...
        logs = {
            ...
            "local_kl_loss_weighted": self.local_kl_weight * local_kl_loss,
            "tv_loss_weighted": self.tv_weight * tv_loss,
            "reg_loss_weighted": self.reg_weight * reg_loss,
            "edr_loss_weighted": self.edr_weight * edr_loss,
    ...
    def encode_inputs(self, partial_pcs, full_pcs=None):
        ...
        if full_pcs is not None:
            ...
            if self.sample_temperature is not None:
                prior_std = prior_std * self.sample_temperature
            ...
            for idx, (cur_reso, cur_c, cur_prior_encoder) in enumerate(zip(self.resolutions[1:], layered_c, self.local_prior_encoders)):
                ...
                if self.sample_temperature is not None:
                    local_prior_stds = local_prior_stds * self.sample_temperature
        ...
        # Maybe we need an attention layer for this mixed feats
        if self.attn_grid:
            pred_feats["grid_c"] = c_local["grid"]
