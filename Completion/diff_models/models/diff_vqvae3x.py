...
from models.diff_cli import GroupVQModel


# @MODEL_REGISTRY
class DiffVQVAE3X(KiloBase):
    """
    ...
    def __init__(self, decoder: nn.Module, local_pointnet: nn.Module, unet3d: UNet3DCLI = None,
                 feat_dim=16, vox_reso=64, emb_weight=0.01, recon_weight=1., latent_dim: int=4,
        ...
        self.vqvae = GroupVQModel(in_channels=feat_dim * 3, out_channels=feat_dim * 3, down_block_types=down_block_types,
            up_block_types=up_block_types, block_out_channels=block_out_channels, layers_per_block=layers_per_block,
            num_vq_embeddings=num_vq_embeddings, feat_split_group=len(plane_types),
        ...
        self.latent_dim = latent_dim # latent_dim = embedding dim x 3
    ...
    def step(self, batch, batch_idx):
        ...
        h = self.vqvae.encode(stacked_x_local).latents
        h_ = rearrange(h, 'b (c g) h w -> (b g) c h w', c=self.latent_dim, g=len(self.plane_types))
        quant, emb_loss, info = self.vqvae.quantize(h_)
        quant = rearrange(quant, '(b g) c h w -> b (c g) h w', c=self.latent_dim, g=len(self.plane_types))

        decoded_feats = self.vqvae.decode(quant, force_not_quantize=True).sample # have already been quantized to obtain the loss
        ...
        loss = self.emb_weight * emb_loss + self.recon_weight * recon_loss

        # Filter loss when NaN was introduced by recon_loss
        if self.filter_nan and torch.any(torch.isnan(loss)):
            console_logger = logging.getLogger("pytorch_lightning")
            console_logger.error(f"NaN loss encountered: loss: {loss}, kl_loss: {emb_loss}, recon_loss: {recon_loss}")
            loss = self.emb_weight * emb_loss
        ...
        if self.interactive_debug:
            print(f"loss: {loss.item()}, kl: {emb_loss.item()}")
    ...
    def encode_inputs(self, partial_pcs, full_pcs=None):
        ...
        if full_pcs is not None:
            x_local = self.local_pointnet(full_pcs)
            stacked_x_local = rearrange([x_local[k] for k in self.plane_types], 'p b f v1 v2 -> b (f p) v1 v2')
            decoded_feats = self.vqvae(stacked_x_local).sample # have already been quantized to obtain the loss
            splited_preds = rearrange(decoded_feats, 'b (f p) v1 v2 -> p b f v1 v2', f=self.feat_dim, p=len(self.plane_types))
            pred_feats.update({k: v for k, v in zip(self.plane_types, splited_preds)})
        else:
            rand_idxs = torch.randint(0, self.vqvae.quantize.n_e, (bs, len(self.plane_types), latent_plane_dim, latent_plane_dim))
            flat_idxs = rearrange(rand_idxs, 'b p v1 v2 -> (b p v1 v2)', p=len(self.plane_types)) 
            flat_feats = self.vqvae.quantize.embedding.weight.data[flat_idxs].detach()
            rand_latents = rearrange(flat_feats, '(b p v1 v2) f -> b (f p) v1 v2', p=len(self.plane_types),
                b=bs, f=self.vqvae.quantize.vq_embed_dim, v1=latent_plane_dim, v2=latent_plane_dim) 

            decoded_feats = self.vqvae.decode(rand_latents, force_not_quantize=True).sample # have already been quantized to obtain the loss
            splited_preds = rearrange(decoded_feats, 'b (f p) v1 v2 -> p b f v1 v2', f=self.feat_dim, p=len(self.plane_types))
            pred_feats.update({k: v for k, v in zip(self.plane_types, splited_preds)})
    ...
    def configure_optimizers(self):
        ...
        if self.pretrained_lr is not None:
            ...
            optimizer = optimizer_cls(self.parameters(), lr=self.lr)
            return optimizer