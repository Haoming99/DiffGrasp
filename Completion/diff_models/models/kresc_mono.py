...
class KResCMono(KiloBase):
    ...
    def step(self, batch, batch_idx):
        ...
        layered_x = [
            repeat(cur_x, "b (s f) vv -> b f (s rc vv)", s=3, rc=self.Rc, f=self.feat_dim)
        ...
        for idx, (cur_reso, cur_x, cur_vae_encoder, cur_prior_encoder) in enumerate(
            zip(self.resolutions[1:], layered_x, self.local_vae_encoders, self.local_prior_encoders)
        ):
        ...
        if self.unet3d is not None:
            if self.fuse_feats:
                sampled_feat_grid = rearrange(
                    [sampled_feat_grid, grid_c], "u b c vx vy vz -> b (u c) vx vy vz"
                )
        ...
        if torch.any(torch.isnan(loss)):
            ...
            console_logger.error(
                f"\nNaN loss encountered: loss: {loss}, kl_loss: {kl_loss}, local_kl_loss: {local_kl_loss}, recon_loss: {recon_loss}, tv_loss: {tv_loss}, reg_loss: {reg_loss}, edr_loss: {edr_loss}\n"
            )
    ...
    def encode_inputs(self, partial_pcs, full_pcs=None):
        ...
        if full_pcs is not None:
            ...
            for idx, (cur_reso, cur_prior_encoder) in enumerate(
                zip(self.resolutions[1:], self.local_prior_encoders)
            ):
                repeated_latents = repeat(
                    repeated_latents,
                    "b c (s rc v) -> b c (s rc v ratio)",
