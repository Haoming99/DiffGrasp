...
from utils.viz import plot_smpl, plot_multi_kpts
from utils.mesh import generate_from_latent
from argparse import ArgumentParser
import math
import matplotlib.pyplot as plt

class SKDUALVAE(pl.LightningModule):
    def __init__(self, enc_in_dim=24*3, enc_c_dim=15*3, enc_hidden_dim=256, latent_dim=256, c_dim=32, dec_type='cbn',
                 ...
                 num_init_layers=0, train_num_samples=1, val_num_samples=4, sk_coeff=1., **kwargs):
        super().__init__()
        self.latent_dim = latent_dim
        self.lr = lr
        self.kl_coeff = kl_coeff
        self.sk_coeff = sk_coeff
        ...
        self.sk_decoder = VAEDecoder(latent_dim, dec_hidden_dim, dec_out_dim, c_dim)
    ...
    def forward(self, c, pts, num_samples=1):
        ...
        return self.decoder(pts, z, c).reshape(bs, num_samples, -1), self.sk_decoder(z, c).reshape(bs, num_samples, -1)
    ...
    def _run_step(self, x, c, pts, num_samples=1):
        ...
        sk_dec_out = self.sk_decoder(z, c)
        sk_dec_out = sk_dec_out.repeat_interleave(num_samples, dim=0)
        # TODO: doesn't really support multiple samples for now
        return z, dec_out, p, q, sk_dec_out
    ...
    def step(self, batch, batch_idx, num_samples=1):
        ...
        z, pred_occ, p, q, pred_sk = self._run_step(x, c, query_pts, num_samples=num_samples)

        recon_loss = F.binary_cross_entropy_with_logits(
            pred_occ, query_occ, reduction='none').sum(-1).mean()

        y = x.repeat_interleave(num_samples, dim=0)
        x_hat = pred_sk.reshape(*y.shape)
        sk_recon_loss = F.l1_loss(x_hat, y, reduction="mean")
        ...
        loss = kl * self.kl_coeff + recon_loss + self.sk_coeff * sk_recon_loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        bs = c.shape[0]
        c = c.reshape(bs, -1)
        pred_occ, pred_sk = self.forward(c, query_pts, num_samples=self.val_num_samples)
        with torch.no_grad():
            ...
            y_sk = batch['full_joints'].repeat_interleave(self.val_num_samples, dim=0).reshape(bs, self.val_num_samples, -1)
            pred_sk = pred_sk.reshape(*y_sk.shape)
            logs['c_sk_recon'] = F.l1_loss(pred_sk, y_sk, reduction="mean")
    ...
    def generate_mesh(self, batch, batch_idx, max_bs=3, num_samples=4, sample=True):
        ...
        partial_joints = partial_joints.repeat_interleave(num_samples, dim=0).reshape(bs * num_samples, -1).to(device)
        x = None if sample else batch['full_joints'].repeat_interleave(num_samples, dim=0).reshape(bs * num_samples, -1).to(device)

        with torch.no_grad():
            z, c = self.encode_input(partial_joints, x)
            multi_recon_joints = self.sk_decoder(z, c).reshape(bs, num_samples, -1, 3)
            ...
            for b_i in range(min(max_bs, bs)):
                cur_mesh_list = list()
                multi_kpts_vis = np.concatenate([batch['full_joints'][None, batch_idx].cpu().numpy(), multi_recon_joints[batch_idx].cpu().numpy()])
                cur_img = plot_multi_kpts(multi_kpts_vis, return_img=True);
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--sk_coeff", type=float, default=1.0)
