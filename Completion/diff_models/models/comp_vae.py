...
@MODEL_REGISTRY
class CompVAE(pl.LightningModule):
    """
    ...
    def __init__(self, decoder: nn.Module, latent_decoder: nn.Module, lr=1e-4, padding=0.1, b_min=-0.5,
                 b_max=0.5, points_batch_size=100000, batch_size=12, test_num_samples=1, kl_weight=0.01,
                cond_dim=32, pts_dim=3, ptn_hidden_dim=256, ptn_out_dim=256, enc_hidden_dim=512, latent_dim=256, 
        ...
        self.partial_pointnet = ResnetPointnet(c_dim=cond_dim, dim=pts_dim, hidden_dim=ptn_hidden_dim)
        if share_ptn:
        ...
        self.encoder = VAEEncoder(ptn_out_dim, enc_hidden_dim, latent_dim, conditioning_channels=cond_dim, batchnorm=batchnorm)
    ...
    def configure_optimizers(self):
        if not self.freeze_decoder:
            ...
            optimizer = torch.optim.Adam([{'params': itertools.chain(self.partial_pointnet.parameters(), self.full_pointnet.parameters(),
                                                         self.encoder.parameters(), self.latent_decoder.parameters())},
