...
class UVAETri(KiloBase):
    ...
    def __init__(self, decoder: nn.Module, partial_local_pointnet: nn.Module, full_local_pointnet: nn.Module, unet3d: UNet3DCLI = None,
        ...
        self.vae_unet = nn.ModuleDict({k: VAEUNet(feat_dim, feat_dim, in_reso=vox_reso, block_type=block_type, activation_type=activation_type, depth=unet_depth, start_filts=unet_filts, z_dim=global_latent_dim) for k in plane_types})
    ...
    def step(self, batch, batch_idx):
        ...
        for plane_type in self.plane_types:
            ...
            px_zc, stats, reg_prob_stats, reg_prior_stats = self.vae_unet[plane_type].forward(x_local[plane_type], c_local[plane_type])
    ...
    def encode_inputs(self, partial_pcs, full_pcs=None):
        ...
        if full_pcs is not None:
            ...
            for plane_type in self.plane_types:
                px_zc = self.vae_unet[plane_type].forward(x_local[plane_type], c_local[plane_type])[0]

                pred_feats[plane_type] = px_zc # p(x|z,c)

        else:
            for plane_type in self.plane_types:
                px_zc = self.vae_unet[plane_type].forward_cond(c_local[plane_type]) # will be repeatede in get_inputs_cond
