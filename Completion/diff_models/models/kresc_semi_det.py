...
class KResCSemiDet(KiloBase):
    ...
    def __init__(self, decoder: nn.Module, full_global_pointnet: nn.Module, local_pointnet: nn.Module,
        ...
        dec_pre_path: Optional[str]=None, residual_latents=True, resolutions=[1, 4, 8, 16], no_cond=False,
        ...
        self.local_vae_encoders = nn.ModuleList([GroupVAEEncoder(in_channels=self.feat_dim,
                                      ...
                                      latent_dim=self.global_latent_dim//2, groups=self.Rc * cur_reso * 3,
    ...
    def step(self, batch, batch_idx):
        """
        partial_pcs is used as full_pcs and full_pcs is discared
        """
        partial_pcs, full_pcs = batch['partial_pcs'], batch['full_pcs']
        query_pts, query_occ = batch['query_pts'], batch['query_occ']

        x_local = self.local_pointnet(partial_pcs)
        x_global = self.full_global_pointnet(partial_pcs)
        ...
        mu, log_var = self.global_vae_encoder(x=x_global, c=None)
        ...
        for idx, (cur_reso, cur_x, cur_vae_encoder) in enumerate(zip(self.resolutions[1:], layered_x, self.local_vae_encoders)):
            ...
            local_zs = torch.cat([local_mus, local_log_vars], dim=1)
        ...
        if not self.fuse_feats and not self.attn_grid and not self.hparams.no_cond:
            sampled_feat_grid = rearrange([sampled_feat_grid, grid_c], 'u b c vx vy vz -> b (u c) vx vy vz')
    ...
    def encode_inputs(self, partial_pcs, full_pcs=None):
        c_global = self.full_global_pointnet(partial_pcs)
