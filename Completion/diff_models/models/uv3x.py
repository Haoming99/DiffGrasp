...
class UVAE3X(KiloBase):
    ...
    def __init__(self, decoder: nn.Module, local_pointnet: nn.Module, unet3d: UNet3DCLI = None,
                 feat_dim=16, vox_reso=16, kl_weight=0.01, recon_weight=1., latent_dim=16, reg_prob_weight=0., reg_prior_weight=0.,
        ...
        self.vae_unet = UNetPlugged(feat_dim * self.num_planes, feat_dim * self.num_planes, in_reso=vox_reso, block_type=block_type, activation_type=activation_type, depth=unet_depth, start_filts=unet_filts, z_dim=latent_dim)
    ...
    def step(self, batch, batch_idx):
        ...
        stacked_c_local = rearrange([c_local[k] for k in self.plane_types], 'p b f v1 v2 -> b (f p) v1 v2')

        px_zc, stats, reg_prob_stats, reg_prior_stats = self.vae_unet.forward(stacked_x_local, stacked_c_local)
        kl_loss = sum([i.sum(dim=(1, 2, 3)) for i in stats]) # get a (b, ) tensor
        kl_loss = kl_loss.mean()

        reg_prob_loss = sum(i.sum(dim=(1, 2, 3)) for i in reg_prob_stats).mean()
        reg_prior_loss = sum(i.sum(dim=(1, 2, 3)) for i in reg_prior_stats).mean()

        splited_preds = rearrange(px_zc, 'b (f p) v1 v2 -> p b f v1 v2', f=self.feat_dim, p=self.num_planes)
    ...
    def encode_inputs(self, partial_pcs, full_pcs=None):
        ...
        if full_pcs is not None:
            x_local = self.local_pointnet(full_pcs)
            stacked_x_local = rearrange([x_local[k] for k in self.plane_types], 'p b f v1 v2 -> b (f p) v1 v2')
            px_zc = self.vae_unet.forward(stacked_x_local, stacked_c_local)[0] # The first return value is feat
        else:
            px_zc = self.vae_unet.forward_cond(stacked_c_local) # will be repeatede in get_inputs_cond
