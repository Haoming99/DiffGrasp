...
class FULL_AE(pl.LightningModule):
    def __init__(self, enc_in_dim=24*3, enc_c_dim=24*3, enc_hidden_dim=256, latent_dim=256, c_dim=32, dec_type='cbn',
    ...
    def step(self, batch, batch_idx, num_samples=1):
        c, x = batch['full_joints'], batch['full_joints']
    ...
    def generate_mesh(self, batch, batch_idx, max_bs=3, num_samples=4, sample=True):
        ...
        partial_joints = batch['full_joints']
        ...
        with torch.no_grad():
            ...
            for b_i in range(min(max_bs, bs)):
                cur_mesh_list = list()
                cur_img = plot_smpl(full_joints[b_i].cpu().numpy(), return_img=True)
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--enc_c_dim", type=int, default=24*3)
