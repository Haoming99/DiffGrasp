...
@MODEL_REGISTRY
class KiloVAE(pl.LightningModule):
    """
    ...
    def __init__(self, decoder: nn.Module, unet3d: UNet3DCLI=None, lr=1e-4, padding=0.1, b_min=-0.5,
        ...
        self.partial_global_pointnet = ResnetPointnet(c_dim=feat_dim, dim=3, hidden_dim=feat_dim*2)
        self.full_global_pointnet = ResnetPointnet(c_dim=feat_dim, dim=3, hidden_dim=feat_dim*2)
        ...
        self.global_vae_encoder = VAEEncoder(feat_dim, feat_dim*2, feat_dim, conditioning_channels=feat_dim, batchnorm=batchnorm)
        self.global_prior_encoder = VAEEncoder(feat_dim, feat_dim*2, feat_dim, conditioning_channels=0, use_conditioning=False, batchnorm=batchnorm)
        
        # Local VAEs, naive implementation
        self.local_vae_encoders = GroupVAEEncoder(feat_dim, feat_dim*2, feat_dim, groups=vox_reso**3, conditioning_channels=feat_dim, use_conditioning=True, batchnorm=batchnorm)
        self.local_prior_encoders = GroupVAEEncoder(feat_dim, feat_dim*2, feat_dim, groups=vox_reso**3, conditioning_channels=0, use_conditioning=False, batchnorm=batchnorm)
    ...
    def step(self, batch, batch_idx):
        ...
        flatten_x_local = rearrange(x_local['grid'], 'b c h w d -> b c (h w d)')
        ...
        repeated_z = repeat(z, 'b c -> b c (h w d)', h=self.vox_reso, w=self.vox_reso, d=self.vox_reso)
