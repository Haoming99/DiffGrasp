...
class KiloPre(KiloBase):
    ...
    def __init__(self, decoder: nn.Module, partial_global_pointnet: nn.Module, partial_local_pointnet: nn.Module,
        ...
        batchnorm=True, dropout=False, reduction='sum', invis_loss_weight=1., interactive_debug=False, freeze_decoder=False, test_num_pts=2048,
        skip_rt=False):
    ...
    #         z = q.rsample()
    #         z = rearrange([mu, log_var], 'u b c -> b (u c)')
            z = mu
    ...
    #             local_zs = rearrange([local_mus, local_log_vars], 'u b c ... -> b (u c)')
                local_zs = local_mus
    ...
    #             local_ps = torch.cat([local_prior_mus, local_prior_log_vars], dim=1)
...
#             local_kl_loss = sum(map(torch.mean, local_kl_losses)) / len(local_kl_losses)
        ...
        local_kl_loss = torch.tensor(0.)
        # TODO: add loss to partial encoder that the features should be similar to gloabl features? 
    ...
    def encode_inputs(self, partial_pcs, full_pcs=None):
        pred_feats = {}
        if self.skip_rt:
            full_pcs = full_pcs if full_pcs is not None else partial_pcs
            x_local = self.full_local_pointnet(full_pcs)
            # sampled_feat_grid = x_local['grid']
            pred_feats = x_local 
            c_global = torch.empty((full_pcs.shape[0], 0), device=full_pcs.device)
        else:
            c_global = self.partial_global_pointnet(partial_pcs)
            c_local = self.partial_local_pointnet(partial_pcs)
    ...
    #             std = torch.exp(log_var / 2)
    #             q = torch.distributions.Normal(mu, std)
    #             z = rearrange([mu, log_var], '2 b c -> b (2 c)')
                z = mu
    ...
    #                 local_zs = rearrange([local_mus, local_log_vars], '2 b c ... -> b (2 c)')
                    local_zs = local_mus
    ...
    #                 local_zs = rearrange([local_prior_mus, local_prior_log_vars], '2 b c ... -> b (2 c)')
                    local_zs = local_prior_mus
    ...
    # Should use ddp to allow checking un used parameters in stead
