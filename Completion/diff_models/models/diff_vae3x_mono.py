...
class DiffVAE3XMono(KiloBase):
    ...
    def __init__(self, decoder: nn.Module, local_pointnet: nn.Module, unet3d: UNet3DCLI = None,
        ...
        self.vae = AutoencoderKL(in_channels=feat_dim * self.num_planes, out_channels=feat_dim * self.num_planes, down_block_types=down_block_types,
    ...
    def step(self, batch, batch_idx):
        ...
        # Add noise to complete posterir to help the model to adapt our corrupting experiment
        for k in pred_feats.keys():
            pred_feats[k] = pred_feats[k] + self.corrupt_mult * torch.randn_like(pred_feats[k])
        kl_loss = posterior.kl().mean()

        # Do the same thing to partials without introducing noise
        partial_posterior = self.vae.encode(stacked_c_local).latent_dist
        pc_z = partial_posterior.sample()
        decoded_partial_feats = self.vae.decode(pc_z).sample
        splited_partial_preds = rearrange(decoded_partial_feats, 'b (f p) v1 v2 -> p b f v1 v2', f=self.feat_dim, p=self.num_planes)
        pred_feats.update({f"{k}_c": v for k, v in zip(self.plane_types, splited_partial_preds)})
        partial_kl_loss = partial_posterior.kl().mean()
        ...
        loss = self.kl_weight * kl_loss + self.recon_weight * recon_loss + self.kl_weight * partial_kl_loss
        ...
        logs = {
            ...
            "kl_loss": kl_loss,
            "partial_kl_loss": partial_kl_loss,
