...
class CondVDVAETri(KiloBase):
    ...
    def __init__(self, decoder: nn.Module, partial_local_pointnet: nn.Module, full_local_pointnet: nn.Module, unet3d: UNet3DCLI = None,
                 feat_dim=16, vox_reso=16, kl_weight=0.01, recon_weight=1., Rc=10, global_latent_dim=16, vdvae_width=64,
                 plane_types=['xy', 'xz', 'yz'], local_cond_dim=32,
        ...
        self.encoders = nn.ModuleDict({k: VDVAE_Encoder(
            feat_dim, vdvae_width, enc_blocks_str, bottleneck_multiple=bottleneck_multiple, custom_width_str=custom_width_str) for k in plane_types})
        self.part_encoders = nn.ModuleDict({k: VDVAE_Encoder(
            feat_dim, vdvae_width, enc_blocks_str, bottleneck_multiple=bottleneck_multiple, custom_width_str=custom_width_str) for k in plane_types})
        self.vdvae_decoders = nn.ModuleDict({k: VDVAE_Decoder(vdvae_width, dec_blocks_str, global_latent_dim,
                                            bottleneck_multiple=bottleneck_multiple, no_bias_above=no_bias_above) for k in plane_types})
    ...
    def step(self, batch, batch_idx):
        ...
        for plane_type in self.plane_types:
            full_activations = self.encoders[plane_type].forward(x_local[plane_type])
            part_activations = self.part_encoders[plane_type].forward(c_local[plane_type])
            px_zc, stats = self.vdvae_decoders[plane_type].forward(
    ...
    def encode_inputs(self, partial_pcs, full_pcs=None):
        ...
        if full_pcs is not None:
            ...
            for plane_type in self.plane_types:
                full_activations = self.encoders[plane_type].forward(x_local[plane_type])
                part_activations = self.part_encoders[plane_type].forward(c_local[plane_type])
                px_zc, _ = self.vdvae_decoders[plane_type].forward(
                    full_activations, part_activations)
            ...
            for plane_type in self.plane_types:
                part_activations = self.part_encoders[plane_type].forward(c_local[plane_type])
                px_zc = self.vdvae_decoders[plane_type].forward_cond(part_activations, n=1) # will be repeatede in get_inputs_cond
