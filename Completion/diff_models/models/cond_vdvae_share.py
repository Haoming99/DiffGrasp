...
class CondVDVAEShare(KiloBase):
    ...
    def __init__(self, decoder: nn.Module, partial_local_pointnet: nn.Module, full_local_pointnet: nn.Module, unet3d: UNet3DCLI = None,
        ...
        self.encoder = VDVAE_Encoder(feat_dim, vdvae_width, enc_blocks_str, bottleneck_multiple=bottleneck_multiple, custom_width_str=custom_width_str)
        self.part_encoder = VDVAE_Encoder(feat_dim, vdvae_width, enc_blocks_str, bottleneck_multiple=bottleneck_multiple, custom_width_str=custom_width_str)
        self.vdvae_decoder = VDVAE_Decoder(vdvae_width, dec_blocks_str, global_latent_dim, bottleneck_multiple=bottleneck_multiple, no_bias_above=no_bias_above)
    ...
    def step(self, batch, batch_idx):
        ...
        for plane_type in self.plane_types:
            full_activations = self.encoder.forward(x_local[plane_type])
            part_activations = self.part_encoder.forward(c_local[plane_type])
    ...
    def encode_inputs(self, partial_pcs, full_pcs=None):
        ...
        if full_pcs is not None:
            ...
            for plane_type in self.plane_types:
                full_activations = self.encoder.forward(x_local[plane_type])
                part_activations = self.part_encoder.forward(c_local[plane_type])
                px_zc, _ = self.vdvae_decoder.forward(
                    full_activations, part_activations)
