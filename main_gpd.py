...
def coord_to_transform(grasp_pose):
    ...
    bottom = grasp_bottom - 0.015 * getApproach
...
subprocess.run(
    ["/home/haoming/anaconda3/envs/3dsg_venv/bin/python3.9", "/home/haoming/Downloads/3DSGrasp-master/Completion/main.py", "--test",
     "--ckpts", "/home/haoming/Downloads/3DSGrasp-master/Completion/3DSGrasp.pth", "--config",
...
red_color = np.array([[1, 0, 0] for _ in range(len(compl_pc.points))])  # RGB: Red

# Assign the color to the point cloud
compl_pc.colors = o3d.utility.Vector3dVector(red_color)
o3d.visualization.draw_geometries([compl_pc, outlier_cloud])
#o3d.visualization.draw_geometries([compl_pc])

merged_xyz = np.concatenate((np.asarray(compl_pc.points), np.asarray(pcd.points)), axis=0)
#merged_xyz = np.asarray(compl_pc.points)
